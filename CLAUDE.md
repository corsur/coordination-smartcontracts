...
├── circuits/
│   └── bool_range/
│       ├── bool_range.circom           # boolean constraint circuit
│       ├── verifying_key.json          # generated by snarkjs
│       └── verifying_key.rs            # generated Rust constant (verifying key bytes)
└── tests/
...
For the reveal hash: use Solana's native `sol_sha256` syscall directly rather than the `sha2` crate — lower compute unit cost.
...
One profile per (wallet, tournament). Created at first `join_game` call; player pays for creation.
...
```
Pending ──(join_game: player 2)──► Active
...
- Validate `end_time > start_time >= Clock::now()`
...
First call (player 1 — creates game):
- Increment `GameCounter.count`; use as `game_id`
- Init Game PDA; set `player_one`, `stake_lamports`, `state = Pending`
- Transfer `stake_lamports` from player to Game PDA
- Create `PlayerProfile` for player 1 if it doesn't exist (player pays)

Second call (player 2 — joins game):
...
- Create `PlayerProfile` for player 2 if it doesn't exist
- Emit `GameStarted`
...
- Slash non-participating player: their stake goes to tournament prize pool
- Return non-slashed player's stake to their wallet
- Update both PlayerProfiles
- Set `state = Resolved`; close Game account (rent to caller as tx fee incentive)
- Emit `TimeoutSlash`
...
```
// Private inputs: guess (0 or 1), salt (field element)
// Public input: commitment = SHA-256(guess_byte || salt_bytes) as field element

template BoolRange() {
    signal private input guess;
    signal private input salt;
    signal output commitment;

    // Constraint: guess ∈ {0, 1}
    guess * (guess - 1) === 0;

    // Commitment matches on-chain reveal hash
    // SHA-256 gadget links circuit output to on-chain sol_sha256 verification
}
```

The commitment output is the public input to the proof and the value stored on-chain as `p1_commit` / `p2_commit`. At reveal time, the program recomputes `sol_sha256(guess || salt)` and checks it matches.

`zk.rs` embeds the verifying key as a `const [u8; N]` and exposes:
